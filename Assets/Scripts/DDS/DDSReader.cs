...
namespace DDS
{
	...
	/// <summary>
	/// Taken from https://github.com/ColeDeanShepherd/TESUnity/blob/f4d5e19f68da380da9da745356c7904f3428b9d6/Assets/Scripts/Core/DDSReader.cs
	/// </summary>
	public static class DDSReader
	{
		...
		/// </summary>
		private static void ExtractDDSTextureFormatAndData(DDSHeader header, BinaryReader reader, out bool hasMipmaps, out uint DDSMipmapLevelCount, out TextureFormat textureFormat, out int bytesPerPixel, out byte[] textureData)
		{
			...
			if(Utils.ContainsBitFlags(header.pixelFormat.flags, (uint)DDSPixelFormatFlags.RGB))
			{
				...
				else
				{
					...
					reader.Read(textureData, 0, (int)remainingByteCount);
				}
			}
			else if(StringUtils.Equals(header.pixelFormat.fourCC, "DXT1"))
			{
				...
				var remainingByteCount = reader.BaseStream.Length - reader.BaseStream.Position;
				var compressedTextureData = reader.ReadBytes((int)remainingByteCount);
